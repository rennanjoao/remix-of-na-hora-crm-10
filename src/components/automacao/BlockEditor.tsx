...
import {
  GripVertical, Trash2, Type, Image as ImageIcon, MousePointerClick,
  MessageCircle, User, Minus,
} from 'lucide-react';
import {
  EmailBlock, BLOCK_LABELS, blocksToHtml, defaultBlockFor,
...
function SortableBlock({
  block, isActive, onClick, onDelete,
}: { block: EmailBlock; isActive: boolean; onClick: () => void; onDelete: () => void }) {
  ...
  if (block.type === 'text') {
    return (
      <div className="space-y-2">
        <Label>Texto (HTML simples permitido)</Label>
        <Textarea rows={8} value={block.html} onChange={(e) => patch('html', e.target.value)} />
      </div>
    );
  }
  if (block.type === 'image') {
...
export function BlockEditor({ blocks, subject, onChange, onSubjectChange }: Props) {
  const [activeId, setActiveId] = useState<string | null>(blocks[0]?.id ?? null);
  const sensors = useSensors(useSensor(PointerSensor, { activationConstraint: { distance: 4 } }));

  const emit = (next: EmailBlock[]) => onChange(next, blocksToHtml(next));
