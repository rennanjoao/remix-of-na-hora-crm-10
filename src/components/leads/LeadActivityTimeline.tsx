              ...
              <div className="rounded-md border bg-card p-2.5">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className="text-[10px] text-muted-foreground">
                    {format(new Date(r.created_at), "dd/MM 'às' HH:mm", { locale: ptBR })}
                  </span>
                </div>
                <p className="text-sm mt-0.5">{r.description}</p>
                {r.action_type === 'status_change' && (r.previous_status || r.new_status) && (
                  <p className="text-xs text-muted-foreground mt-1">
                    {r.previous_status ?? '—'} → <span className="font-medium text-foreground">{r.new_status ?? '—'}</span>
                  </p>
                )}
