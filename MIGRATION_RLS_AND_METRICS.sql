...
-- ETAPA 1: Corrigir vazamento de leads entre SDRs (RLS)
-- ETAPA 3: Índices + view sdr_performance + get_sdr_performance +
--          lock/claim_next_lead + Realtime das tabelas do Command Center
-- Aplique este arquivo INTEIRO no editor SQL da Lovable Cloud (uma vez).
-- =====================================================================

-- ---------- ETAPA 1: RLS de leads ----------
...
-- ---------- ETAPA 3.a: Índices de performance ----------
...
-- ---------- ETAPA 3.b: View de performance por dia ----------
...
LEFT JOIN LATERAL (
  SELECT
    COUNT(*) AS total,
    COUNT(*) FILTER (WHERE importado) AS importados
  FROM public.cnpj_consultas cc
  WHERE cc.consultado_por = p.id
    AND date_trunc('day', cc.created_at) = d.dia
) consultas ON true
LEFT JOIN LATERAL (
...
-- ---------- ETAPA 3.c: RPC de leitura (admin/gerente) ----------
...
-- ---------- ETAPA 3.d: Lock de concorrência + claim_next_lead ----------
...
-- ---------- ETAPA 3.e: Realtime para o Command Center ----------
