...
DROP FUNCTION IF EXISTS public.sdr_work_queue();

CREATE OR REPLACE FUNCTION public.sdr_work_queue()
 RETURNS TABLE(item_key text, source text, bucket text, priority integer, lead_id uuid, lead_name text, lead_cnpj text, lead_city text, lead_state text, lead_phone text, lead_email text, lead_status text, title text, due_at timestamp with time zone, extra jsonb)
 LANGUAGE plpgsql
...
BEGIN
  ...
  SELECT
    ...
    l.id, COALESCE(l.nome_fantasia, l.razao_social), l.cnpj, l.cidade, l.estado,
  ...
  WHERE m.lead_id IS NOT NULL
    AND m.status IN ('agendada','confirmada','scheduled','confirmed')
